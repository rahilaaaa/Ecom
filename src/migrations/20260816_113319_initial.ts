...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_products_badge" AS ENUM('none', 'new', 'sale');
  ...
  CREATE TYPE "public"."enum__products_v_version_badge" AS ENUM('none', 'new', 'sale');
  ...
  CREATE TYPE "public"."enum_orders_payment_method" AS ENUM('cod', 'stripe');
  ...
  CREATE TYPE "public"."enum_footer_customer_care_link_type" AS ENUM('reference', 'custom');
  ...
  CREATE TABLE "variants" (
  	...
  	"price_in_i_n_r_enabled" boolean DEFAULT true,
  ...
  CREATE TABLE "_variants_v" (
  	...
  	"version_price_in_i_n_r_enabled" boolean DEFAULT true,
  ...
  CREATE TABLE "orders" (
  	...
  	"checkout_idempotency_key" varchar,
  	"estimated_delivery_from" timestamp(3) with time zone,
  	"estimated_delivery_to" timestamp(3) with time zone,
  ...
  CREATE TABLE "header" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"announcement" varchar DEFAULT 'FREE SHIPPING ON ORDERS OVER $150',
  ...
  CREATE TABLE "footer_customer_care" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  	"link_type" "enum_footer_customer_care_link_type" DEFAULT 'reference',
  	"link_new_tab" boolean,
  	"link_url" varchar,
  	"link_label" varchar NOT NULL
  );
  ...
  CREATE TABLE "footer" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"description" varchar DEFAULT 'Redefining modern minimalism through emotional design and uncompromising quality.',
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  ...
  CREATE TABLE "homepage" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"hero_eyebrow" varchar DEFAULT 'THE NEW STANDARD',
  	"hero_heading" varchar DEFAULT 'Spring Collection 2024' NOT NULL,
  	"hero_cta_label" varchar DEFAULT 'SHOP NOW',
  	"hero_cta_url" varchar DEFAULT '/shop',
  	"hero_media_id" integer,
  	"curated_heading" varchar DEFAULT 'Curated For You',
  	"best_sellers_heading" varchar DEFAULT 'Best Sellers',
  	"best_sellers_view_all_label" varchar DEFAULT 'View All Products',
  	"best_sellers_limit" numeric DEFAULT 4,
  	"testimonial_quote" varchar DEFAULT 'Elixir has completely redefined my wardrobe. The attention to detail and the quality of the fabrics are simply unmatched in today''s market. It is quiet luxury at its finest.',
  	"testimonial_attribution" varchar DEFAULT 'SARAH JENKINS, VOGUE EDITOR',
  	"newsletter_heading" varchar DEFAULT 'Join the Inner Circle',
  	"newsletter_description" varchar DEFAULT 'Sign up to receive early access to new collections, exclusive events, and curated editorial content.',
  	"newsletter_form_id" integer,
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  
  CREATE TABLE "discover_popular_searches" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  	"label" varchar NOT NULL,
  ...
  CREATE TABLE "discover" (
  	...
  	"new_arrivals_description" varchar DEFAULT 'Explore the latest seasonal pieces.',
  	"new_arrivals_href" varchar DEFAULT '/shop?badge=new',
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  ...
  ALTER TABLE "footer_customer_care" ADD CONSTRAINT "footer_customer_care_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."footer"("id") ON DELETE cascade ON UPDATE no action;
  ...
  ALTER TABLE "homepage" ADD CONSTRAINT "homepage_newsletter_form_id_forms_id_fk" FOREIGN KEY ("newsletter_form_id") REFERENCES "public"."forms"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "discover_popular_searches" ADD CONSTRAINT "discover_popular_searches_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."discover"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE UNIQUE INDEX "orders_checkout_idempotency_key_idx" ON "orders" USING btree ("checkout_idempotency_key");
  ...
  CREATE INDEX "footer_customer_care_order_idx" ON "footer_customer_care" USING btree ("_order");
  CREATE INDEX "footer_customer_care_parent_id_idx" ON "footer_customer_care" USING btree ("_parent_id");
  ...
  CREATE INDEX "homepage_newsletter_newsletter_form_idx" ON "homepage" USING btree ("newsletter_form_id");
  CREATE INDEX "discover_popular_searches_order_idx" ON "discover_popular_searches" USING btree ("_order");
  CREATE INDEX "discover_popular_searches_parent_id_idx" ON "discover_popular_searches" USING btree ("_parent_id");`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "footer_customer_care" CASCADE;
  DROP TABLE "footer_legal" CASCADE;
  DROP TABLE "footer_nav_items" CASCADE;
  DROP TABLE "footer" CASCADE;
  DROP TABLE "footer_rels" CASCADE;
  DROP TABLE "homepage" CASCADE;
  DROP TABLE "discover_popular_searches" CASCADE;
  ...
  DROP TYPE "public"."enum_footer_customer_care_link_type";
